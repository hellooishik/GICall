...
class Settings {
    ...
    public function init_settings() {
        ...
        if (isset($_POST['fbcn_restore_defaults']) && current_user_can('manage_options')) {
            if (isset($_POST['fbcn_restore_nonce']) && wp_verify_nonce($_POST['fbcn_restore_nonce'], 'fbcn_restore_defaults_nonce')) {
    ...
    private function sanitize_time_window($input) {
        ...
        $sanitized['start_time'] = preg_match('/^([0-1][0-9]|2[0-3]):[0-5][0-9]$/', $start_time) ? $start_time : '00:00';
        $sanitized['end_time'] = preg_match('/^([0-1][0-9]|2[0-3]):[0-5][0-9]$/', $end_time) ? $end_time : '23:00';
            ...
            foreach ($valid_days as $day) {
                if (isset($input['time_windows'][$day])) {
                    ...
                    $sanitized['time_windows'][$day] = array(
                        'start_time' => preg_match('/^([0-1][0-9]|2[0-3]):[0-5][0-9]$/', $day_start) ? $day_start : '00:00',
                        'end_time' => preg_match('/^([0-1][0-9]|2[0-3]):[0-5][0-9]$/', $day_end) ? $day_end : '23:00'
                    );
                } else {
                    ...
                    <form method="post" action="options.php" class="fbcn-main-form">
                        ...
                        <div class="fbcn-card">
                            ...
                            <div class="fbcn-card-footer" style="display: flex; gap: 10px; align-items: center;">
                                ...
                                <button type="submit" name="fbcn_restore_defaults" value="1" class="button button-secondary large" formaction="<?php echo esc_url(admin_url('admin.php?page=' . esc_attr($_GET['page']))); ?>" formnovalidate onclick="return confirm('<?php _e('Are you sure you want to restore all settings to defaults?', 'fb-call-now'); ?>');">
                                    <?php _e('Restore Defaults', 'fb-call-now'); ?>
                                </button>
                                <?php wp_nonce_field('fbcn_restore_defaults_nonce', 'fbcn_restore_nonce', true, true); ?>
                            </div>
                        </div>
                    </form>
                </div>
                                ...
                                <div class="fbcn-pro-field-group">
                                    <div class="fbcn-pro-field-label">
                                        ...
                                        <div class="fbcn-input-wrapper" style="position: relative; display: inline-block;">
                                            <span class="dashicons dashicons-editor-help" onmouseenter="document.getElementById('timezone_tooltip').style.display='block';" onmouseleave="document.getElementById('timezone_tooltip').style.display='none';" style="cursor:help; margin-left:4px; font-size:16px;"></span>
                                            <div id="timezone_tooltip" class="fbcn-tooltip" style="display: none; position: absolute; left: 105%; top: 50%; transform: translateY(-50%); background: #333; color: white; padding: 6px 10px; border-radius: 4px; font-size: 12px; white-space: nowrap; z-index: 10;">
                                                <?php esc_html_e( 'Time settings are applied using your WordPress site timezone setting.', 'fb-call-now' ); ?>
                                                <div style="position: absolute; right: 100%; top: 50%; transform: translateY(-50%); border-width: 5px; border-style: solid; border-color: transparent #333 transparent transparent;"></div>
                                            </div>
                                        </div>
                                    </div>
                                    <table class="fbcn-time-table" style="width:100%; max-width:500px; margin-bottom: 15px;">
                                        ...
                                        <tr class="fbcn-time-row" style="margin-bottom: 10px; display: flex; align-items: center; gap: 10px;">
                                            ...
                                            <td class="fbcn-time-field"><label style="display:none;"><?php _e( 'Start', 'fb-call-now' ); ?></label><input type="time" name="fbcn_pro_settings[time_windows][<?php echo $key; ?>][start_time]" class="fbcn-time-select" value="<?php echo esc_attr($day_start); ?>" /></td>
                                            <td class="fbcn-time-divider" style="padding-top:0;"><span class="dashicons dashicons-arrow-right-alt"></span></td>
                                            <td class="fbcn-time-field"><label style="display:none;"><?php _e( 'End', 'fb-call-now' ); ?></label><input type="time" name="fbcn_pro_settings[time_windows][<?php echo $key; ?>][end_time]" class="fbcn-time-select" value="<?php echo esc_attr($day_end); ?>" /></td>
                                        </tr>
                                        <?php endforeach; ?>
                                    </table>
        ...
        <table class="form-table">
            <tr>
                <td>
                    ...
                    <input type="time" name="fbcn_pro_settings[start_time]" id="start_time" value="<?php echo esc_attr($start_time); ?>" />
                </td>
                <td>
                    <label for="end_time"><?php _e('End Time:', 'fb-call-now'); ?></label>
                    <input type="time" name="fbcn_pro_settings[end_time]" id="end_time" value="<?php echo esc_attr($end_time); ?>" />
                </td>
            </tr>
        </table>
