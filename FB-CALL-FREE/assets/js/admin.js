...
(function ($) {
    ...
    function updatePreview() {
        ...
        var isBottom = vertPos > 5;
        var topPercent = 'auto';
        var bottomPercent = 'auto';
        
        if (isBottom) {
            bottomPercent = (((10 - vertPos) / 9) * 85) + 5; // e.g. at 10 -> 5%, at 6 -> ~42%
            bottomPercent = bottomPercent + '%';
        } else {
            topPercent = (((vertPos - 1) / 9) * 85) + 5; // e.g. at 1 -> 5%, at 5 -> ~42%
    ...
    $('form').on('submit', function (e) {
        // Skip validation if submitting via "Restore Defaults"
        if (e.originalEvent && e.originalEvent.submitter && e.originalEvent.submitter.name === 'fbcn_restore_defaults') {
            return true;
        }
